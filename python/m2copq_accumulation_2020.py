...
# 累計excel資料處理
# 20200708 加上yearmonthLog設定
#path = 'D:/RMA需求/COPQ/2020 月圖 美金_公版_0528(加實績版 For 0604).xlsx'
path = 'D:/RMA需求/COPQ/2020 月圖 美金_公版.xlsx'
...
if sheet_raw.nrows > 0 :
    cur.execute("delete from rma.copq_accumulation_actachievement where yearmonthLog=%s",(yearmonthLog))   
    for row in range(1, sheet_raw.nrows):    
        ...
        cur.execute("insert ignore into rma.copq_accumulation_actachievement(yearmonthLog,yearmonth,app,area,m2copq_target,copq_target,copq,m2copq,provision,`purge`,mcr)values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(yearmonthLog,yearmonth,app,area,m2copq_target,copq_target,copq,m2copq,provision,purge,mcr))
cur.execute("commit")       